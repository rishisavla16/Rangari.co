...
for (const [key, value] of Object.entries({ home, about, footer })) {
  await sql.query('INSERT INTO site_content (content_key, value, updated_at) VALUES ($1, $2::jsonb, now()) ON CONFLICT (content_key) DO UPDATE SET value = EXCLUDED.value, updated_at = now()', [key, JSON.stringify(value)]);
}
